...
def main():
    ...
    # get ploidy
    #@CFraisse_2021: hard code ploidy
    ploidy = 2
    print('Ploidy is:', ploidy)

    # are genotypes phased
    #@CFraisse_2021: hard code phasing
    phased = True
    if not phased:
    ...
    if phased:
        if blend:
            ...
            for gffrec in gff[gene][feat]:
                ...
                # in case of indels
                posadd = 0
                #@CFraisse_2023: try/except = mark as missing data locus present in the GFF & FASTA but not in the VCF
                try:
                    ...
                    for sample in seqs.keys(): tmpseqs[sample] = "??????????"
                    print('Locus present in the GFF & FASTA but not in the VCF ----- missing data')
            ...
            for gffrec in gff[gene][feat]:
                ...
                try:
                    ...
                    for sample in seqs.keys(): seqs[featname][sample] = "??????????"
                    print('Locus present in the GFF & FASTA but not in the VCF ----- missing data')
                feat_ind += 1
                if strand == "-":
...
def getAlleles(rec, ploidy):
    ...
    #@CFraisse_2021: to allow missing data in invariants
    if None in segregating and len(segregating)==1:
        max_len = 1
    else:
...
#@CFraisse_2021: hardcoded otherwise issue with invariants
#def getPloidy(vcf):
